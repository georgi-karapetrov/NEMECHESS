...
void PositionResolver::resolve() //global resolve which won't work, like ever
{
  /*  bool nothingMoved = false;
    ...
    {
        for ( QVector< ChessPiece* >::iterator i = m_board->m_pieces.begin(); i != m_board->m_pieces.end(); ++i )
        {
            unsigned int count = 0;
            QVector< Position >::iterator j = ( *i )->allowedMovements().begin();
            QVector< Position > tmp = ( *i )->allowedMovements();
